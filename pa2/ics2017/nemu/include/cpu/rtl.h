...
/* PA 2: 辅助函数
 * 根据操作数宽度返回一个 C 层面的辅助值，
 * 服务于后续 RTL 伪指令或执行函数，只读取操作数宽度内的有效位。
 */
static inline rtlreg_t rtl_mask(int width) {
...
#define make_rtl_setget_eflags(f) \
  static inline void concat(rtl_set_, f) (const rtlreg_t* src) { \
    /* 标志位 RTL 辅助函数只把规范化后的 0/1 写入 CPU 位域。 */ \
    cpu.f = *src & 0x1; \
  } \
...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  rtlreg_t val = *src1;
  switch (width) {
    case 1: *dest = (int8_t)val; return;
    case 2: *dest = (int16_t)val; return;
    case 4: *dest = (int32_t)val; return;
    default: assert(0);
  }
}
...
static inline void rtl_push(const rtlreg_t* src1) {
  // esp <- esp - 4
  // M[esp] <- src1
  /* 先保存一份本地副本。这样可以符合 i386 对 PUSH ESP 的规定：
   * 压栈的是旧 ESP 值，而不是递减后的栈指针。
   */
  rtlreg_t val = *src1;
  rtl_subi(&cpu.esp, &cpu.esp, 4);
  rtl_sm(&cpu.esp, 4, &val);
}

static inline void rtl_pop(rtlreg_t* dest) {
  // dest <- M[esp]
  // esp <- esp + 4
  /* 在更新栈指针和写目标之前，先把栈顶值读到临时变量中。
   * 这样也能保证 POP ESP 正确：最终 ESP 应该等于弹出的值。
   */
  rtlreg_t val;
  rtl_lm(&val, &cpu.esp, 4);
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  ...
  /* 判断零标志前先按操作数宽度截断，因为字节/字操作可能在临时结果
   * 的高位留下与本次运算无关的值。
   */
  rtlreg_t val = *result & rtl_mask(width);
  rtl_eq0(&val, &val);
  rtl_set_ZF(&val);
}
