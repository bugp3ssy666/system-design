...
void difftest_step(uint32_t eip) {
  ...
  // PA 2: QEMU 的 8 个 GDB 通用寄存器顺序构建和 cpu.gpr[] 一致
  int i;
  for (i = R_EAX; i <= R_EDI; i ++) {
    if (reg_l(i) != r.array[i]) {
      printf("DIFF at eip = 0x%08x: %s, NEMU = 0x%08x, QEMU = 0x%08x\n",
          eip, reg_name(i, 4), reg_l(i), r.array[i]);
      diff = true;
    }
  }

  // EIP 不在 cpu.gpr[] 内，需要单独比较
  if (cpu.eip != r.eip) {
    printf("DIFF at eip = 0x%08x: eip, NEMU = 0x%08x, QEMU = 0x%08x\n",
        eip, cpu.eip, r.eip);
    diff = true;
  }
