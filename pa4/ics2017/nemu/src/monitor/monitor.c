...
static inline void restart() {
  ...
  /* [PA 4] 手册要求初始 CR0 为 0x60000011；PG 位仍为 0，之后由内核写 CR0 开启分页 */
  cpu.cr0.val = 0x60000011;
  cpu.cr3.val = 0;

  /* [PA 3] 初始运行在 Nexus-AM 使用的平坦内核代码段中 */
  cpu.cs = 8;
  cpu.idtr.limit = 0;
  cpu.idtr.base = 0;

  /* PA 2: i386 复位状态要求 EFLAGS 的 [1] 位恒为 1，并清空这里建模的状态标志。
   * 后续改变标志位的指令只通过 RTL helper 更新相应的位。
   */
  /* [PA 3] 中断入口会压入 EFLAGS，因此复位时保持体系结构要求的 bit 1 为 1 */
