...
/* PA 2: 补全 group 和 opcode_table 注册各个指令 */

/* group 1: 立即数/ModR/M 共享的算术逻辑指令组 */
make_group(gp1,
...
/* group 4: r/m8 形式的 INC/DEC */
make_group(gp4,
    EX(inc), EX(dec), EMPTY, EMPTY,
    EMPTY, EMPTY, EMPTY, EMPTY)

/* group 5: r/m 形式的 INC/DEC/CALL/JMP/PUSH */
make_group(gp5,
    EX(inc), EX(dec), EX(call_rm), EMPTY,
    EX(jmp_rm), EMPTY, EX(push), EMPTY)

/* [PA 3] 系统指令组中 0f 01 /3 对应 LIDT m16&32 */
make_group(gp7,
...
void exec_wrapper(bool print_flag) {
  ...
  /* [PA 4] 每条指令结束后轮询 INTR；IF 为 1 时响应 32 号时钟中断。 */
  if (cpu.INTR && cpu.IF) {
    cpu.INTR = false;
