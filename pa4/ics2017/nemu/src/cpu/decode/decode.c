...
static inline make_DopHelper(SI) {
   ...
   *
   op->simm = ???
   */
  /* PA 2: 有符号立即数会用于 CALL/JMP 的相对偏移，也会用于 opcode 0x83
   * 的 group-1 ALU 指令。先按编码宽度取出立即数，再按照 i386 手册
   * 对 rel32/imm8 形式的要求进行符号扩展。
   */
  uint32_t imm = instr_fetch(eip, op->width);
  op->simm = (op->width == 1 ? (int8_t)imm : (int32_t)imm);
...
static inline void decode_mov_cr(vaddr_t *eip, bool r2cr) {
  ModR_M m;
  m.val = instr_fetch(eip, 1);

  /* [PA 4] MOV CRx 只支持寄存器操作数，编码要求 ModR/M.mod == 3 */
  assert(m.mod == 3);
...
make_DHelper(mov_r2cr) {
  decode_mov_cr(eip, true);
}

make_DHelper(mov_cr2r) {
  decode_mov_cr(eip, false);
}
...
make_DHelper(push_SI) {
  ...
  /* PUSH imm8 的编码只有 1 字节立即数，但真正压栈前需要先按操作数宽度做符号扩展 */
  id_dest->width = 1;
